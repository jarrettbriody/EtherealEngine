...
BullEnemy::~BullEnemy()
{
	delete bt;
	int index = (rand() % 6);
	Config::FMODResult = Config::FMODSystem->playSound(Config::BullDeath[index], Config::SFXGroup, false, &Config::SFXChannel);
	Config::SFXChannel->setVolume(BULL_DEATH_VOLUME * Config::SFXVolume);
